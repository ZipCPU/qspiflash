...
#define	PARENT	WBFLASH_TB<Vqflexpress>
...
class	QFLEXPRESS_TB : public PARENT {
	FLASHSIM	*m_flash;
	bool		m_bomb;
public:
	...
	void	tick(void) {
		...
		{ static int lastsck = 0; int iqspi;
			...
			if (lastsck)
				(*m_flash)(m_core->o_qspi_cs_n, 0,
					m_core->o_qspi_dat);

			iqspi = (*m_flash)(m_core->o_qspi_cs_n, 1,
				m_core->o_qspi_dat);
	...
	void	place_online(void) {
		...
		// 3 address bytes
		cfg_write(CFG_USERMODE | CFG_QSPEED | CFG_WEDIR);
		cfg_write(CFG_USERMODE | CFG_QSPEED | CFG_WEDIR);
		cfg_write(CFG_USERMODE | CFG_QSPEED | CFG_WEDIR);
		// mode byte
		cfg_write(CFG_USERMODE | CFG_QSPEED | CFG_WEDIR | 0xa0);
		// Read a dummy byte
...
int main(int  argc, char **argv) {
	...
	QFLEXPRESS_TB	*tb = new QFLEXPRESS_TB;
	const char	*DEV_RANDOM = "/dev/urandom";
	unsigned	rdv;
	unsigned	*rdbuf;

	tb->opentrace("qflexpress.vcd");
