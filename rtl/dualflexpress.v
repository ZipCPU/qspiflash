...
`ifdef	VERIFIC
	...
	sequence	READ_WORD;
		...
		(o_wb_stall)&&(o_dspi_sck)&&(clk_ctr == 6'd16)
	...
	sequence	DSPI_CFG_READ_SEQ;
		((o_wb_stall)&&(!o_dspi_cs_n)
			&&(o_dspi_mod == DUAL_READ)&&(!o_wb_ack)
			&&(cfg_mode)&&(cfg_speed)&&(!cfg_dir))
			throughout
		((clk_ctr == 6'd5)&&(o_dspi_sck))
		##1 ((clk_ctr== 6'd4)&&(o_dspi_sck))
		##1 ((clk_ctr== 6'd3)&&(o_dspi_sck))
		##1 ((clk_ctr== 6'd2)&&(o_dspi_sck))
	...
	property SPI_CFG_WRITE;
		...
		##1 ((o_wb_ack)||(!$past(pre_ack))||($past(!i_wb_cyc)))
			&&(o_wb_data[7]==$past(i_dspi_dat[1],8))
			&&(o_wb_data[6]==$past(i_dspi_dat[1],7))
			&&(o_wb_data[5]==$past(i_dspi_dat[1],6))
			&&(o_wb_data[4]==$past(i_dspi_dat[1],5))
			&&(o_wb_data[3]==$past(i_dspi_dat[1],4))
			&&(o_wb_data[2]==$past(i_dspi_dat[1],3))
			&&(o_wb_data[1]==$past(i_dspi_dat[1],2))
			&&(o_wb_data[0]==$past(i_dspi_dat[1]))
			&&(o_wb_data[12:10]==3'b100)&&(o_wb_data[8]);
	endproperty

	// Config read-HS  request
	property DSPI_CFG_READ;
		disable iff (i_reset)
		(cfg_hs_read)
		|=> DSPI_CFG_READ_SEQ
		##1 ((o_wb_ack)||(!$past(pre_ack))||($past(!i_wb_cyc)))
	...
	assert	property (@(posedge i_clk) DSPI_CFG_READ);
	assert	property (@(posedge i_clk) DSPI_CFG_WRITE);
