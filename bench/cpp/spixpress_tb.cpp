...
unsigned flreadid(SPIXPRESS_TB *tb) {
	unsigned	r;

	tb->cfg_write(F_READID);

	tb->cfg_write(0); r = tb->cfg_read() & 0x0ff;
	tb->cfg_write(0); r = (r<<8) | (tb->cfg_read() & 0x0ff);
	tb->cfg_write(0); r = (r<<8) | (tb->cfg_read() & 0x0ff);
	tb->cfg_write(0); r = (r<<8) | (tb->cfg_read() & 0x0ff);
	tb->cfg_write(F_END);
	return r;
...
int	flstatus(SPIXPRESS_TB *tb) {
	tb->cfg_write(F_RDSR);
	tb->cfg_write(0);
	tb->cfg_write(F_END);
	return tb->cfg_read() & 0x0ff;
}

void	flwait(SPIXPRESS_TB *tb) {
	int	r;
...
void	flerase(SPIXPRESS_TB *tb, unsigned sectoraddr) {
	tb->cfg_write(F_END);
	tb->cfg_write(F_WREN);
	tb->cfg_write(F_END);

	tb->cfg_write(F_SE);
	tb->cfg_write((sectoraddr >> 16)&0x0ff);
	tb->cfg_write((sectoraddr >>  8)&0x0ff);
	tb->cfg_write((sectoraddr      )&0x0ff);
	tb->cfg_write(F_END);

	flwait(tb);
}

void	flpage_program(SPIXPRESS_TB *tb, int addr, int ln, const char *buf) {
	flwait(tb);
	...
	tb->cfg_write(F_PP);
	tb->cfg_write((addr >> 16)&0x0ff);
	tb->cfg_write((addr >>  8)&0x0ff);
	tb->cfg_write((addr      )&0x0ff);

	// Write the page data itself
	for(int i=0; i<ln; i++)
		tb->cfg_write(buf[i] & 0x0ff);
	tb->cfg_write(F_END);
...
void	flprogram(SPIXPRESS_TB *tb, int addr, int ln, const char *buf) {
	int	start = addr;
	...
	while(start < addr + ln) {
		...
		printf("PAGEOF (end) = %08x\n", PAGEOF(addr+ln-1));
		printf("PAGEOF (now) = %08x\n", PAGEOF(start));
		printf("length chosen= %8x\n", wlen);
		printf("start = %8x\n", start);
		printf("addr  = %8x\n", addr);
		printf("ln    = %8x\n", ln);
assert(wlen <= PGLENB);
		flpage_program(tb, start, wlen, &buf[start-addr]);
...
int main(int  argc, char **argv) {
	...
	for(int i=0; (i<1000)&&(!tb->bombed()); i++) {
		...
		if(tblv != rdv) {
			printf("BOMB: READ/SINGLE %08x, EXPECTED %08x\n", rdv, tblv);
			goto test_failure;
			break;
		} // else printf("MATCH: %08x == %08x\n", rdv, tblv);
	} tb->wb_read(1000<<2, 1000, rdbuf);
	...
	for(int i=0; i<1000; i++) {
		if ((*tb)[i+1000] != rdbuf[i]) {
			printf("BOMB: READ/VECTOR %08x, EXPECTED %08x\n", rdv, (*tb)[i+1000]);
			goto	test_failure;
		}
	} printf("VECTOR TEST PASSES!\n");
	...
	for(int i=0; (i<1000)&&(!tb->bombed()); i++) {
		...
		if(tblv != rdv) {
			printf("BOMB: READ/XIP %08x, EXPECTED %08x\n", rdv, tblv);
			goto test_failure;
			break;
		} // else printf("MATCH: %08x == %08x\n", rdv, tblv);
	}
	...
	for(int i=0; i<1000; i++) {
		if ((*tb)[i+1000] != rdbuf[i]) {
			printf("BOMB: READ/XIP/VECTOR %08x, EXPECTED %08x\n", rdv, (*tb)[i+1000]);
			goto	test_failure;
		}
	} printf("VECTOR TEST PASSES!\n");
	...
	for(int p=0; p<NPAGES; p++) {
		...
		flprogram(tb, SECTORSZB+p*SZPAGEB, SZPAGEB,
